...
namespace word_doc_mvp.Services
{
    public static class DocxNormalizerService
    {
        ...
        private static void NormalizeAllParts(WordprocessingDocument doc, Action<string> log)
        {
            ...
            var mainXDoc = LoadXDocument(mainPart);

            log?.Invoke("Stripping RSID attributes...");
            StripRsidAttributes(mainXDoc);

            log?.Invoke("Removing proofing error markers...");
            RemoveElements(mainXDoc, W + "proofErr");

            log?.Invoke("Removing bookmarks...");
            RemoveElements(mainXDoc, W + "bookmarkStart");
            RemoveElements(mainXDoc, W + "bookmarkEnd");

            log?.Invoke("Removing comments and comment references...");
            RemoveElements(mainXDoc, W + "commentRangeStart");
            RemoveElements(mainXDoc, W + "commentRangeEnd");
            RemoveElements(mainXDoc, W + "commentReference");

            log?.Invoke("Removing smart tags...");
            UnwrapElements(mainXDoc, W + "smartTag");

            log?.Invoke("Accepting tracked revisions...");
            AcceptRevisions(mainXDoc);

            log?.Invoke("Nullifying w14:paraId and w14:textId...");
            NullifyW14Identifiers(mainXDoc);

            log?.Invoke("Merging adjacent fragmented runs...");
            MergeAdjacentRuns(mainXDoc);

            log?.Invoke("Hoisting namespace declarations to root...");
            HoistNamespacesToRoot(mainXDoc);

            SaveXDocument(mainPart, mainXDoc);
