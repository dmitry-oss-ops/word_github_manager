    ...
    public static class DocxNormalizerService
    {
        ...
        private static void NormalizeAllParts(WordprocessingDocument doc, Action<string> log)
        {
            ...
            if (mainPart.DocumentSettingsPart != null)
            {
                ...
                RemoveElements(settingsXDoc, W + "useFELayout");
        ...
        #region Volatile Drawing Elements

        /// <summary>
        /// Strips optional layout-hint elements that Word injects on save when it
        /// re-evaluates drawing anchor sizing (e.g. after any edit near a shape).
        /// These have no visual effect when pctWidth/pctHeight is 0.
        /// </summary>
        private static void StripVolatileDrawingElements(XDocument xdoc)
        {
            xdoc.Descendants(WP14 + "sizeRelH").ToList().ForEach(e => e.Remove());
            xdoc.Descendants(WP14 + "sizeRelV").ToList().ForEach(e => e.Remove());
        }

        #endregion

        #region VML Style Sanitization

        private static readonly HashSet<string> VolatileVmlStyleProps = new HashSet<string>(
        ...
        /// <summary>
        /// Removes volatile mso-width-relative and mso-height-relative properties
        /// from VML style attributes. Word injects these when it re-evaluates
        /// drawing layout, producing noise even when the shape hasn't changed.
        /// </summary>
        private static void SanitizeVmlStyles(XDocument xdoc)
        {
            foreach (var element in xdoc.Descendants().ToList())
            {
                ...
                var cleaned = parts
                    .Where(p =>
                    {
                        var propName = p.Split(':')[0].Trim();
                        return !VolatileVmlStyleProps.Contains(propName);
                    })
                    .ToList();

                if (cleaned.Count < parts.Count)
                {
                    styleAttr.Value = string.Join(";", cleaned);
                }
            }
        }
        ...
        private static void NormalizeXmlPart(OpenXmlPart part)
        {
            ...
            StripGfxData(xdoc);
            StripVolatileDrawingElements(xdoc);
            SanitizeVmlStyles(xdoc);
