    ...
    public class GitHubService
    {
            ...
            Action<string> log = null)
        {
            try
            {
                ...
                {
                    log?.Invoke($"Contents API returned {contents[0].Content.Length:N0} characters.");
                    return contents[0].Content;
                }

                log?.Invoke("Content field empty (file likely > 1 MB), falling back to Blob API...");
            }
            ...
            {
                log?.Invoke($"Contents API failed ({ex.GetType().Name}), falling back to Blob API...");
            }
            ...
            Action<string> log)
        {
            ...
            log?.Invoke($"Downloading blob (SHA: {treeItem.Sha}, size: {treeItem.Size:N0} bytes)...");
            ...
            if (blob.Encoding == EncodingType.Base64)
            {
                ...
                log?.Invoke($"Blob decoded: {result.Length:N0} characters.");
                return result;
            }

            log?.Invoke($"Blob returned as UTF-8: {blob.Content?.Length ?? 0:N0} characters.");
            return blob.Content;
        }
    }
}
